...
class SocketController extends Controller implements MessageComponentInterface
{
    ...
    public function onOpen(ConnectionInterface $conn)
    {
        ...
        if (isset($queryarray['token'])) {
            ...
            $datass = User::where('userId', $queryarray['token'])->update(['connection_id' => $conn->resourceId, 'user_status' => 'Online']);


            $user_id = User::select('userId')->where('userId', $queryarray['token'])->get();
    ...
    public function onMessage(ConnectionInterface $conn, $msg)
    {
        ...
        if (isset($data->type)) {
            ...
            if ($data->type == 'request_load_unconnected_user') {
                $user_type = User::where('userId', trim($data->from_user_id, " "))->first();
                if ($user_type->Type_user == 'Practice') {
                    $user_data = User::where('userId', '!=', trim($data->from_user_id, " "))->where('Type_user', 'Provider')
                        ->orderBy('name', 'ASC')
                        ->get();

                      $data_count = User::select('userId')->where('userId', '!=', trim($data->from_user_id, " "))
                        ->where('Type_user', 'Provider')
                        ->orderBy('name', 'ASC')
                        ->get();

                        $data_cht =array();
                      foreach($data_count as $chats)
                      {
                        $data_cht[] = Chat::where('message_status', '!=', 'Read')->where('from_user_id',trim($chats->user_Id," "))->get();
                    ...
                    foreach ($user_data as $row) {
                        ...
                        if (date('Y-m-d') == date('Y-m-d', strtotime($row->updated_at))) {
                            $dt = new DateTime($row->updated_at);
                            $tz = new DateTimeZone('Asia/Kolkata');

                            $dt->setTimezone($tz);
                            //                                    
                            $last_seen = $dt->format('Y-m-d h:i:a');
                        } else {
                            $dt = new DateTime($row->updated_at);
                            $tz = new DateTimeZone('Asia/Kolkata');
                    ...
                    $sender_connection_id = User::select('connection_id')->where('userId', trim($data->from_user_id, " "))->get();
                ...
                if ($user_type->Type_user == 'Provider') {
                    $user_data = User::
                        where('userId', '!=', trim($data->from_user_id, " "))->where('Type_user', 'Practice')
                        ->orderBy('name', 'ASC')
                        ->get();
            ...
            if ($data->type == 'request_search_user') {
                $user_type = User::where('userId', trim($data->from_user_id, " "))->first();
                if ($user_type->Type_user == 'Practice') {
                    $user_data = User::where('userId', '!=', trim($data->from_user_id, " "))
                                        ->where('Type_user', 'Provider')
                ...
                if ($user_type->Type_user == 'Provider') {
                    $user_data = User::select('userId', 'name', 'user_status', 'user_image')
                        ->where('userId', '!=', trim($data->from_user_id, " "))
                        ->where('Type_user', 'Practice')
                        ->where('name', 'like', '%' . $data->search_query . '%')
                        ->orderBy('name', 'ASC')
                        ->get();
                    ...
                    //                      $q->where('id', '!=', $data->from_user_id)
                    //                 //   ->where('name', 'like', '%'.$data->search_query.'%')
                    //                   ->orWhere('LOWER(name)','like','%'.strtolower($data->search_query).'%');
            ...
            if ($data->type == 'request_load_unread_notification') {
                $notification_data = Chat_request::select('id', 'from_user_id', 'to_user_id', 'status')
                    ...
                    ->where(function ($query) use ($data) {
                        $query->where('from_user_id', trim($data->user_id, " "))->orWhere('to_user_id', trim($data->user_id, " "));
                    })->orderBy('id', 'ASC')->get();
                ...
                */
                ...
                foreach ($notification_data as $row) {
                    ...
                    if ($row->from_user_id == trim($data->user_id, " ")) {
                    ...
                    $user_data = User::select('name', 'user_image')->where('userId', $user_id)->first();
                ...
                $sender_connection_id = User::select('connection_id')->where('userId', trim($data->user_id, " "))->get();
            ...
            if ($data->type == 'request_process_chat_request') {
                ...
                $sender_connection_id = User::select('connection_id')->where('userId', $data->from_user_id)->get();

                $receiver_connection_id = User::select('connection_id')->where('userId', $data->to_user_id)->get();
            ...
            if ($data->type == 'request_connected_chat_user') {
                $condition_1 = ['from_user_id' => trim($data->from_user_id, " "), 'to_user_id' => trim($data->from_user_id, " ")];
                ...
                */
                ...
                foreach ($user_id_data as $user_id_row) {
                    ...
                    $user_data = User::select('userId', 'name', 'user_image', 'user_status', 'updated_at')->where('userId', $user_id)->first();
            ...
            if ($data->type == 'request_send_message') {
                ...
                $chat_data = Chat::select('id', 'from_user_id', 'to_user_id', 'chat_message', 'message_status')
                    ->where(function ($query) use ($data) {
                        $query->where('from_user_id', $data->from_user_id)->where('to_user_id', trim($data->to_user_id, " "));
                    })
                    ->orWhere(function ($query) use ($data) {
                        $query->where('from_user_id', trim($data->to_user_id, " "))->where('to_user_id', trim($data->from_user_id, " "));
                    })->orderBy('id', 'ASC')->get();
                ...
                */
                ...
                $sender_connection_id = User::select('connection_id')->where('userId', trim($data->to_user_id, " "))->get();
                $receiver_connection_id = User::select('connection_id')->where('userId', trim($data->from_user_id, " "))->get();
            ...
            if ($data->type == 'request_chat_history') {
                $chat_data = Chat::select('id', 'from_user_id', 'to_user_id', 'chat_message', 'message_status')
                    ->where(function ($query) use ($data) {
                        $query->where('from_user_id', trim($data->from_user_id, " "))->where('to_user_id', trim($data->to_user_id, " "));
                    })
                    ->orWhere(function ($query) use ($data) {
                        $query->where('from_user_id', trim($data->to_user_id, " "))->where('to_user_id', trim($data->from_user_id, " "));
                    })->orderBy('id', 'ASC')->get();
            ...
            if ($data->type == 'check_unread_message') {
                $chat_data = Chat::select('id', 'from_user_id', 'to_user_id')->where('message_status', '!=', 'Read')->where('from_user_id', trim($data->to_user_id, " "))->get();
                ...
                */
                ...
                $sender_connection_id = User::select('connection_id')->where('userId', trim($data->from_user_id, " "))->get(); //send number of unread message

                $receiver_connection_id = User::select('connection_id')->where('userId', trim($data->to_user_id, " "))->get(); //send message read status
            ...
            if ($data->type == 'userTyping') {
                ...
                $send_data['userTyping'] = ["typing" => "Typing.......", 'count' => $data->Count];

                $sender_connection_id = User::select('connection_id')->where('userId', $data->to_user_id)->get();
            ...
            if ($data->type == 'count_unread_message') {
                $count_data = Chat::select('id', 'from_user_id', 'to_user_id')->where('message_status', '!=', 'Read')->where('from_user_id', $data->to_user_id)->get()->count();
    ...
    public function onClose(ConnectionInterface $conn)
    {
        ...
        if (isset($queryarray['token'])) {
            ...
            User::where('userId', trim($queryarray['token'], " "))->update(['connection_id' => 0, 'user_status' => 'Offline']);

            $user_id = User::select('userId', 'updated_at')->where('userId', trim($queryarray['token'], " "))->get();
